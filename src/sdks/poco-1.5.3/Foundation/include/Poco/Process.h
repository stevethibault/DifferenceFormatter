...
public:
	...
	bool isValid() const;
		/// Checks whether the process handle is valid.
	...
	bool tryWait(int timeoutInMilliseconds) const;
		/// Waits for the process to terminate or timeout is reached.
...
public:
	...
	static bool tryWait(const ProcessHandle& handle, int timeoutInMilliseconds);
		/// Waits for the process specified by handle to terminate or timeout is reached.

	static int exitCode(const ProcessHandle& handle);
		/// Returns the exit code of the process.
