                ...
                TestDetails const& details)
{
    ...
    { 
        ...
        stream << " actual >= (expected - tolerance) " << (actual >= (expected - tolerance)) << "";
        stream << " actual <= (expected + tolerance) " << (actual <= (expected + tolerance)) << "";
...
template< typename Expected, typename Actual >
void CheckTypesEqual(TestResults& results, const Actual& actual, TestDetails const& details)
{
    if (typeid(Expected) != typeid(actual))
    { 
        UnitTest::MemoryOutStream stream;
		stream << "Expected type " << typeid(Expected).name() << " but was type " << typeid(actual).name();
